...
//      let grey =  new Array(outputSlices[0].length).fill(0); // for 2d CC
        ...
        for (let x = 0; x < steps; x++) {
            for (let y = 0; y < steps; y++) {
                for (let z = 0; z < steps; z++) {
                    grid_coords[counter] = Array(gap + x*subcubeside, gap + y*subcubeside, gap + z*subcubeside);
                    counter += 1;
                }}}
        return grid_coords;
    };

    inside = (xyz, cubeside, sub) => {
        for (var i = 0; i < 3; i++) {
            if (Math.sign(xyz[i]) < 0 || xyz[i]+sub > cubeside) {
                return false;
            };
        };
        return true;
    ...
    cloud = (num, mean, sigma, cubeside, subcubeside) => {
        ...
        for (var i = 0; i < num; i++){
            ...
            if( !inside(coord, cubeside, subcubeside) ){
                i--;
            }else{
                coords[i] = coord;
            };
